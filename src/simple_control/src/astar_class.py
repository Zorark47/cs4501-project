...
class AStarPlanner:
  ...
  def plan(self, map_data, drone_position, goal_position):
    ...
    prev = {}
    cost = {}
    prev[drone_position] = None
    cost[drone_position] = 0
    ...
    while not frontier.empty():
      ...
      for next in self.get_neighbors(current, map_data):
        ...
        next = tuple(next)
        new_cost = cost[current] + self.dist(current, next)
        # Compute the estimated cost to goal (heuristic)
        if (next not in cost.keys()) or (new_cost < cost[next]):
        # Add the neighbors to the frontier with the new cost
          cost[next] = new_cost
          priority = new_cost + self.dist(goal_position, next)
          frontier.put((priority, next))
          prev[next] = current
    # Return the path
    path = self.build_path(current, prev)
    return path
      ...
      for y_dim in range(-1, 2):
        ...
        if not (x_dim == y_dim):
          ...
          if (0 <= pos[0] + x_dim < map_in.shape[0]) and(0 <= pos[1] + y_dim < map_in.shape[1]):
            ...
            if map_in[pos[0] + x_dim][pos[1] + y_dim] in [-3, -2, -1, 0]:
  ...
  def build_path(self, current, prev):
    path = []
