...
class LocalPlanner:
    ...
    def setDoor(self):
        ...
        for i in range(len(self.lidar.ranges)):
            if abs(self.prev_scan.ranges[i] - self.lidar.ranges[i]) > min_noise:
                print("I think this is a door" + str(i))

        self.prev_scan = self.lidar.ranges
